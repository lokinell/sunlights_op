...
(function () {
    ...
    PurchaseStatisticsService = (function () {
        ...
        PurchaseStatisticsService.prototype.findTradeSummaries = function (pager) {
            var deferred;
            this.$log.info("trade summaries " + (angular.toJson(pager, true)));
            deferred = this.$q.defer();
            this.$http.get(baseUrl + '/statistics/summaries', {
                headers: {
                    'params': encodeURIComponent(angular.toJson(pager))
                }
            }).success((function (_this) {
                return function (data, status, headers) {
                    _this.$log.info("Successfully find summaries - status " + status);
                    return deferred.resolve(data);
                };
            })(this)).error((function (_this) {
                return function (data, status, headers) {
                    _this.$log.error("Failed to find summaries - status " + status);
                    return deferred.reject(data);
                };
            })(this));
            return deferred.promise;
